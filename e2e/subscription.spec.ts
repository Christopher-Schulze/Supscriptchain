...
test.beforeAll(async () => {
  ...
  const poorSigner = provider.getSigner(poorUser);
  ...
  const subscriptionFactory = new ethers.ContractFactory(
    subJson.abi,
    subJson.bytecode,
    ownerSigner,
  );
  ...
  frontend = spawn('npm', ['run', 'dev', '--', '-p', '3000'], {
    ...
    env: {
      ...process.env,
      NEXT_PUBLIC_CONTRACT_ADDRESS: await subscription.getAddress(),
...
test('complete flow', async ({ page }) => {
  ...
  const before = (await subscription.userSubscriptions(user, 0))
    .nextPaymentDate;
...
test('fails to subscribe with insufficient balance', async ({ page }) => {
  await page.addInitScript(
    ({ rpc, account }) => {
  ...
  await expect(page.locator('.error')).toContainText('exceeds balance');
});
